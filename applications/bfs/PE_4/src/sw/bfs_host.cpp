...
void BuildDisCSR (void)
{
  ...
  uint tailIdx = (node_num/16+2) * 4; // 16 = 4*4
  for (k=0; k<NUM_PE; ++k)
  {
...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  string krnl_name_full = "inst_bfs_4x1_v1:{inst_bfs_4x1_v1_1}";
  ...
  q.enqueueMigrateMemObjects({
    buffer_CSR_V[0], buffer_CSR_V[1], buffer_CSR_V[2], buffer_CSR_V[3],
    buffer_CSR_E[0], buffer_CSR_E[1], buffer_CSR_E[2], buffer_CSR_E[3]}, 0);
  q.finish();
  ...
  q.enqueueMigrateMemObjects (
    { buffer_glbl_d[0], buffer_glbl_d[1], buffer_glbl_d[2], buffer_glbl_d[3]}, CL_MIGRATE_MEM_OBJECT_HOST);  
  q.finish();
