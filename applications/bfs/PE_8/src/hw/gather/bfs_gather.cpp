...
void BFS_Gather(const uint8_t peID, volatile uint32_t *sw_data,
                volatile bool *exist, dpkt_t tmp_dist[2][4096])
{
...
#pragma HLS pipeline II=2
  ...
  rltDstIdx= (dstVid-peID) / (8*8); // 8-Banking
  rltBank  = rltDstIdx % 2;
  rltAddr  = rltDstIdx / (2*8); // 8 data packed

  dstVal  = tmp_dist[rltBank][rltAddr];

  switch (rltDstIdx/2 % 8)
  {
    case 0:
