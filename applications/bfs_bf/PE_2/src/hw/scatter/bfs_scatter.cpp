  ...
  L2: for (uint16_t i=0; i<n_iter; i++)
  {
    ...
    *rd_start = 1;
    ...
    #pragma HLS pipeline II=1
      ...
      {
        srcIdx = tmp_edge[0].range(23, 0); // using rltIdx (to PE) when building CSR_V
...
// 64 URAMs for temporary distance
...
#pragma HLS bind_storage variable=mem_req_buff[1] type=ram_2p impl=bram
  ...
  uint n_vertex_iter = N_Vertex/(2*BATCH_SIZE);
  uint rlt_tail_idx  = N_Vertex/2 + 1;
  ...
  *launch_recv = 0; ////
  ...
  // 2 partition, 8 bit distance packed as 64 bit, 128bit = 64bit *2, each time write 128 items
  if (resWrite == 1)
  {
  ...
  if (*glblIterIdx == 0)
  {
    ...
    if (root % 2 == peID)
    {
      ...
      tmp_dist[rootBank][rootIdx/(64*8)] = wrVal; // dist[root] = 0
    }
  }
  ...
  *launch_recv = 1;
  ...
  if (*glblIterIdx == 0) {
    *rd_baseAddr = (uint64_t)CSR_V;
    BuffOfst(rd_port, rd_req, rd_start, ofst_buff, N_Vertex/8+2); // 8=2*4
  }
  *rd_baseAddr = (uint64_t)CSR_E;

  uint16_t n_tuple_0 = 0;
  uint16_t n_tuple_1 = 0;
  // main loop, each L1 iteration traverse BATCH_SIZE*4 vertices
