...
#pragma HLS bind_storage variable=mem_req_buff[1] type=ram_2p impl=bram
  ...
  uint n_vertex_iter = N_Vertex/(16*BATCH_SIZE);
  uint rlt_tail_idx  = N_Vertex/16 + 1;
  ...
  *launch_recv = 0; ////
  ...
  // 16 partition, 8 bit distance packed as 64 bit, 128bit = 64bit *2, each time write 64 items
  if (resWrite == 1)
  {
    uint16_t n_wr_iter = (N_Vertex%(16*8*2*64)==0) ? N_Vertex/(16*8*2*64) : N_Vertex/(16*8*2*64)+1;
  ...
  *launch_recv = 1;
  ...
  if (*glblIterIdx == 0) {
    *rd_baseAddr = (uint64_t)CSR_V;
    BuffOfst(rd_port, rd_req, rd_start, ofst_buff, N_Vertex/64+2); // 64=16*4
  }
