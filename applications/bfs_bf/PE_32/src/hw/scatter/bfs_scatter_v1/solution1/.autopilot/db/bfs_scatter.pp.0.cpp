# 1 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 375 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.cpp" 2
# 1 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.h" 1
...
# 5 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.h" 2
...
# 6 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.h" 2
...
# 7 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.h" 2
...
# 8 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.h" 2
...
# 9 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.h" 2
...
# 10 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.h" 2
...
__attribute__((sdx_kernel("BFS_Scatter", 0))) void BFS_Scatter (
  ...
  ap_uint<64> tmp_dist[8][4096]);
}
# 2 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.cpp" 2





void WrDist (hls::stream<trans_pkt> &wr_port, const ap_uint<64> tmp_dist[8][4096], const uint8_t idx)
{
...
#pragma HLS pipeline II=4
 wr_pkt.data = (tmp_dist[1][rd_baseAddr+i], tmp_dist[0][rd_baseAddr+i]);
    wr_port.write(wr_pkt);

    wr_pkt.data = (tmp_dist[3][rd_baseAddr+i], tmp_dist[2][rd_baseAddr+i]);
    wr_port.write(wr_pkt);

    wr_pkt.data = (tmp_dist[5][rd_baseAddr+i], tmp_dist[4][rd_baseAddr+i]);
    wr_port.write(wr_pkt);

    wr_pkt.data = (tmp_dist[7][rd_baseAddr+i], tmp_dist[6][rd_baseAddr+i]);
    wr_port.write(wr_pkt);
  }
...
void feed_network(hls::stream<trans_pkt> &rd_port, hls::stream<uint32_t> &rd_req,
  ...
  ap_uint<64> tmp_dist[8][4096], hls::stream<uint32_t> &sw_data_0, hls::stream<uint32_t> &sw_data_1,
...
#pragma HLS pipeline II=1
 rd_pkt = rd_port.read();
      ...
      if (tmp_edge[0].range(31,24) == ap_uint<8>(0))
      {
        ...
        srcVal = tmp_dist[srcIdx%8][srcIdx/(8*8)];
        switch ((srcIdx/8) % 8) {
          case 0:
...
__attribute__((sdx_kernel("BFS_Scatter", 0))) void BFS_Scatter (
  ...
  volatile bool resWrite,
  ap_uint<64> tmp_dist[8][4096])
{_ssdm_SpecArrayDimSize(tmp_dist, 8);
#pragma HLS TOP name=BFS_Scatter
# 256 "/Orion/BackEnd/ArtifactEvaluation/BFS_BF/PE_32/src/hw/scatter/bfs_scatter.cpp"
...
#pragma HLS bind_storage variable=mem_req_buff[1] type=ram_2p impl=bram
 ...
 const ap_uint<8> ZERO = 0;
  ...
  int n_vertex_iter = N_Vertex/(32*64);
  int rlt_tail_idx = N_Vertex/32 + 1;
  ...
  *launch_recv = 0;
  ...
  if (resWrite == 1)
  {
    uint16_t n_wr_iter = (N_Vertex%(32*8*2*64)==0) ? N_Vertex/(32*8*2*64) : N_Vertex/(32*8*2*64)+1;
...
#pragma HLS protocol fixed
 _ssdm_op_Wait(1);
  ...
  if (*glblIterIdx == 0)
  {
    ...
    {
      INIT_L0_1: for (uint8_t k=0; k<8; ++k) {
#pragma HLS unroll factor=8
 tmp_dist[k][i] = MASK;
    ...
    if (root%32 == peID)
    {
      rootIdx = (root-peID) / 32;
      rootBank = rootIdx % 8;
      rangeOfst= (rootIdx/8) % 8;
      ...
      tmp_dist[rootBank][rootIdx/(8*8)] = wrVal;
    }
  }
  ...
  *launch_recv = 1;
  ...
  if (*glblIterIdx == 0) {
    *rd_baseAddr = (uint64_t)CSR_V;
    BuffOfst(rd_port, rd_req, rd_start, ofst_buff, N_Vertex/128+2);
  }
